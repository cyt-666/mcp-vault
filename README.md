...
当前版本：<code>0.2.4</code>（2026-09-11）。部署示例默认使用
<code>mcp-vault:0.2.4</code>，目标架构为 <code>linux/amd64</code>。
...
~~~bash
docker build --platform linux/amd64 --tag mcp-vault:0.2.4 --tag mcp-vault:latest .
~~~
...
~~~bash
docker run --rm --platform linux/amd64 --read-only --tmpfs /tmp:rw,noexec,nosuid,nodev,size=16m mcp-vault:0.2.4 --check-config
~~~

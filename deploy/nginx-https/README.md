...
```bash
tar -xzf mcp-vault-nginx-https-0.1.17.tar.gz
cd nginx-https
```
...
```bash
docker load --input ../mcp-vault-0.1.17-linux-amd64.tar
docker image inspect mcp-vault:0.1.17 --format '{{.Os}}/{{.Architecture}} {{.Id}}'
```

...
```bash
tar -xzf mcp-vault-nginx-https-0.2.3.tar.gz
cd nginx-https
```
...
```bash
docker load --input ../mcp-vault-0.2.3-linux-amd64.tar
docker image inspect mcp-vault:0.2.3 --format '{{.Os}}/{{.Architecture}} {{.Id}}'
```

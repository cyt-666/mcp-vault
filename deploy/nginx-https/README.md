...
```bash
tar -xzf mcp-vault-nginx-https-0.2.2.tar.gz
cd nginx-https
```
...
```bash
docker load --input ../mcp-vault-0.2.2-linux-amd64.tar
docker image inspect mcp-vault:0.2.2 --format '{{.Os}}/{{.Architecture}} {{.Id}}'
```

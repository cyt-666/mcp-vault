...
```bash
tar -xzf mcp-vault-nginx-https-0.2.4.tar.gz
cd nginx-https
```
...
```bash
docker load --input ../mcp-vault-0.2.4-linux-amd64.tar
docker image inspect mcp-vault:0.2.4 --format '{{.Os}}/{{.Architecture}} {{.Id}}'
```

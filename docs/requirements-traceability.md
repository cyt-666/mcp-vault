...
| Auth separation, OAuth discovery, and secret redaction | Auth/MCP/WebDAV/Admin tests; real-HTTP built-in DCR/login/duplicate-form-retry/code-PKCE/token/refresh/replay flow and OAuth `create_note`; deterministic full-scope OAuth `tools/list` plus every MCP tool handler; RFC 8414/9728 metadata and challenge tests; local token Vault/resource/scope/rotation tests; optional RS256 `aud`/resource and Vault-grant tests; Admin dual-cookie/CSRF reload and logout tests; redacted diagnostics tests | Complete built-in wire boundary and OAuth tool surface automated; Admin and Vault OAuth credentials remain separate, session bearer remains HttpOnly, and no secret-log finding is allowed; live ChatGPT UI/account login remains manual evidence |
...
| Migration from prior prerelease state | `pre_wp02.sql` upgrade test through migration 0012; focused 0009, 0010, destructive 0011 memory-cutover, and additive 0012 OAuth-schema checks | Automated; preserve fixture and migration checksums |

-- Preserve old Core intents as explicit terminal records when an Admin has
-- confirmed that the legacy memory namespace is being discarded.
CREATE TABLE operation_journal_v3 (
    id TEXT PRIMARY KEY,
    vault_id TEXT NOT NULL,
    operation TEXT NOT NULL,
    state TEXT NOT NULL CHECK (
        state IN (
            'prepared', 'file_committed', 'metadata_committed', 'rolled_back',
            'needs_review', 'superseded', 'discarded'
        )
    ),
    source_path TEXT,
...
INSERT INTO operation_journal_v3 (
...
ALTER TABLE operation_journal_v3 RENAME TO operation_journal;

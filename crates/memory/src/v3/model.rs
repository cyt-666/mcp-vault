...
/// The useful role of a complete memory unit, not a claim of verified truth.
...
/// Current unit selected by an authenticated operator as the basis of a new explicit assertion.
#[derive(Clone, Copy, Debug, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct MemoryCopySource {
    pub id: MemoryId,
...
/// The adapter derives this from the authenticated principal. Internal Admin
/// operations may use All; memory-only credentials use ExplicitOnly.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
...
pub struct RecallRequest {
    /// Authenticated access to original note content.
    pub access: MemoryReadAccess,
    /// Explicit source path filter; absent means all authorized sources.
...
pub struct MemoryUnit {
    ...
    /// Single source set when this unit is automatically selected.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub note_set_id: Option<MemorySetId>,
    /// Optimistic memory metadata revision.
    pub revision: Revision,
    /// Complete body and required source context.
    pub content: String,
    /// Importance.
    pub importance: Option<f64>,
    /// Confidence.
    ...
    /// Entities.
    pub entities: Vec<String>,
    /// Number of current distinct provenance records.
...
pub struct RecallResult {
    ...
    /// Complete units omitted for size; read them by ID without clipping.
    ...
    /// Optional scoped counts and retrieval admission policy.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Navigation to a complete current unit that did not fit the recall envelope.
#[derive(Clone, Debug, PartialEq, Serialize)]

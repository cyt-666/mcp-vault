...
use mcp_vault_state::{
    UnitBundle, UnitOwnership, UnitRecord, UnitSourceRecord, UnitSourceSetRecord,
};
...
const MAX_NOTE_SET_BYTES: usize = 8 * 1024 * 1024;
...
pub fn explicit_path(
    ...
    reserved_root
        .join(
            &VaultPath::parse(&format!("memory-v3/explicit/{memory_id}.md"))
...
pub fn note_set_path(
    ...
    reserved_root
        .join(
            &VaultPath::parse(&format!("memory-v3/sources/{source_file_id}.md"))
...
pub fn render_explicit(bundle: &UnitBundle) -> Result<Vec<u8>, MemoryError> {
    let memory = &bundle.memory;
    let mut output = String::new();
    output.push_str("---\n");
    output.push_str("schema: \"mcp-vault-memory-unit/v3\"\n");
...
pub fn render_note_set(
    ...
    let mut output = String::new();
    output.push_str("---\n");
    output.push_str("schema: \"mcp-vault-memory-units/v3\"\n");
    ...
    output.push_str(&format!(
        "items: {}\n",
        quote(&serde_json::to_string(&canonical_items).map_err(|_| MemoryError::Markdown)?)
    ));
    output.push_str("---\n\n");
    output.push_str(&display_units(set, &canonical_items));
    if output.len() > MAX_NOTE_SET_BYTES {
        return Err(MemoryError::GeneratedOutput(
            "memory_canonical_set_too_large",
        ));
    }
    Ok(output.into_bytes())
}

fn display_units(set: &UnitSourceSetRecord, items: &[CanonicalNoteItem]) -> String {
    let mut output = format!("# 自动记忆：{}\n\n", set.source_path);
    for item in items {
        output.push_str(&format!("## 记忆 {}\n\n", item.id));
...
pub fn parse_explicit(
...
) -> Result<UnitBundle, MemoryError> {
    let (root, body) = parse_document(bytes, MAX_EXPLICIT_BYTES)?;
    if required_string(&root, "schema")? != "mcp-vault-memory-unit/v3"
    ...
    let content_hash = markdown::hash_content(&body);
    Ok(UnitBundle {
        memory: UnitRecord {
            id,
            vault_id,
...
pub fn parse_note_set(
    ...
    canonical_revision: Revision,
    now: i64,
) -> Result<(UnitSourceSetRecord, Vec<UnitBundle>), MemoryError> {
    let (root, body) = parse_document(bytes, MAX_NOTE_SET_BYTES)?;
    if required_string(&root, "schema")? != "mcp-vault-memory-units/v3" {
        return Err(MemoryError::Markdown);
    }
    ...
    let items: Vec<CanonicalNoteItem> = serde_json::from_str(&required_string(&root, "items")?)
        .map_err(|_| MemoryError::Markdown)?;
    if body != display_units(&set, &items) {
        return Err(MemoryError::Markdown);
    }
    if items.len() > 4096 {
        return Err(MemoryError::Markdown);
    ...
    for (index, item) in items.into_iter().enumerate() {
        ...
        if markdown::hash_content(&content) != item.content_hash {
            return Err(MemoryError::Markdown);
        }
        let unit: crate::units::SourceUnit = serde_json::from_value(
            item.metadata
                .get("source_unit")
                .cloned()
                .ok_or(MemoryError::Markdown)?,
        ...
        let source = UnitSourceRecord {
            ...
            heading_path: unit.headings.clone(),
            start_line: Some(unit.body.start_line),
            end_line: Some(unit.body.end_line),

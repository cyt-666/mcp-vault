//! Source-preserving v3 memory persistence.
//! Reuses revision-aware source publication without any formal merger or legacy reads.
...
mod initialization;
mod work;
pub use initialization::{MemoryInitializationState, MemoryInitializationTask};
pub use work::{UnitOverviewRecord, UnitOverviewScope, UnitRuntimeState};

const MAX_MEMORY_LIMIT: u32 = 200;

/// Ownership determines which object controls replacement and deletion.
#[derive(Clone, Copy, Debug, Deserialize, Serialize, Eq, PartialEq)]
pub enum UnitOwnership {
...
pub struct UnitBundle {
    /// Current item.
    pub memory: UnitRecord,
    /// Current provenance.
    pub sources: Vec<UnitSourceRecord>,
    /// Owning set for note-derived items.
    pub note_set: Option<UnitSourceSetRecord>,
}

/// One source note's single current memory set.
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq)]
pub struct UnitSourceSetRecord {
    /// Stable set identity.
    pub id: MemorySetId,
    /// Vault isolation boundary.
    pub vault_id: VaultId,
...
pub struct UnitSourceSetSnapshotRecord {
...
pub struct UnitFilter {
    ...
    pub ownership: Vec<UnitOwnership>,
    ...
    /// Explicit directory boundary with path-segment semantics.
    pub path_prefix: Option<String>,
    /// All requested existing knowledge-map topic keys must match a source.
...
pub struct UnitExplicitReservation {
...
pub struct UnitRepository {
    ...
    initialization_token: Option<mcp_vault_domain::MaintenancePermitToken>,
}

impl UnitRepository {
    pub(crate) fn new(pool: SqlitePool, offline_exclusive: bool) -> Self {
        Self {
            pool,
            offline_exclusive,
    ...
    pub(crate) fn new_for_initialization(
        pool: SqlitePool,
        initialization_token: mcp_vault_domain::MaintenancePermitToken,
    ) -> Self {
        Self {
            pool,
            offline_exclusive: false,
            initialization_token: Some(initialization_token),
        }
    }

    pub(crate) fn initialization_allowed(&self) -> bool {
        self.offline_exclusive
            || self
                .initialization_token
                .as_ref()
                .is_some_and(mcp_vault_domain::MaintenancePermitToken::is_valid)
    }
    ...
    pub async fn list_source_sets(
        ...
        limit: u32,
        offset: u32,
    ) -> Result<Vec<UnitSourceSetView>, StateError> {
        self.ensure_vault_context(context).await?;
        validate_page(limit, offset)?;
        let sql = format!(
            "SELECT s.source_file_id AS file_id, COALESCE(f.path,s.source_path) AS path, s.set_revision, s.extraction_paused AS paused, (SELECT count(*) FROM memory_units i WHERE i.vault_id=s.vault_id AND i.note_set_id=s.id AND {}) AS current_item_count, (f.deleted_at IS NULL AND f.id IS NOT NULL AND lower(f.path) LIKE '%.md') AS restorable FROM memory_unit_sets s LEFT JOIN file_entries f ON f.vault_id=s.vault_id AND f.id=s.source_file_id WHERE s.vault_id=? AND (? IS NULL OR s.extraction_paused=?) ORDER BY s.source_path,s.source_file_id LIMIT ? OFFSET ?",
    ...
    pub async fn get(
        ...
        context: &VaultContext,
        memory_id: MemoryId,
    ) -> Result<Option<UnitBundle>, StateError> {
        self.ensure_vault_context(context).await?;
        let sql = format!(
    ...
    /// Restore only derived search rows without changing unit generations.
    pub async fn rebuild_search_row(
        &self,
        context: &VaultContext,
        item: &UnitRecord,
    ) -> Result<(), StateError> {
        ...
        if item.vault_id != context.id() {
            return Err(StateError::InvalidInput(
                "memory search row belongs to another Vault",
            ));
        }
        let mut tx = self.pool.begin_with("BEGIN IMMEDIATE").await?;
        let current:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM memory_units WHERE vault_id=? AND id=? AND revision=? AND content_hash=?)")
            .bind(context.id().to_string()).bind(item.id.to_string()).bind(item.revision.as_i64()?).bind(&item.content_hash).fetch_one(&mut *tx).await?;
        if !current {
            return Err(StateError::Conflict);
        }
        delete_item_fts(&mut tx, context.id(), item.id).await?;
        insert_item_fts(&mut tx, item).await?;
        tx.commit().await?;
        Ok(())
    }
    ...
    pub async fn count_filtered(
        ...
        self.ensure_vault_context(context).await?;
        let mut query =
            QueryBuilder::<Sqlite>::new("SELECT count(*) FROM memory_units i WHERE i.vault_id=");
        query.push_bind(context.id().to_string());
        query.push(" AND ");
        query.push(contribution_eligibility_sql());
        append_filter(&mut query, filter);
        let count: i64 = query.build_query_scalar().fetch_one(&self.pool).await?;
        u64::try_from(count).map_err(|_| StateError::InvalidInput("memory count is invalid"))
    }
    ...
    pub async fn list(
        ...
        limit: u32,
        offset: u32,
    ) -> Result<Vec<UnitRecord>, StateError> {
        validate_page(limit, offset)?;
        self.ensure_vault_context(context).await?;
        let mut query = QueryBuilder::<Sqlite>::new(item_select());
    ...
    pub async fn counts(&self, context: &VaultContext) -> Result<UnitCounts, StateError> {
        self.ensure_vault_context(context).await?;
        let sql = format!(
            "SELECT i.ownership, COUNT(*) FROM memory_units i\n\
        ...
        for (ownership, count) in rows {
            ...
            match UnitOwnership::parse(&ownership)? {
                UnitOwnership::Explicit => counts.explicit = count,
                UnitOwnership::NoteDerived => counts.note_derived = count,
            }
        }
        let paused: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM memory_unit_sets\n\
    ...
    pub async fn search_fts(
        ...
        filter: &UnitFilter,
        limit: u32,
    ) -> Result<Vec<UnitSearchHit>, StateError> {
        ...
        let mut query = QueryBuilder::<Sqlite>::new(
            "SELECT i.id, i.vault_id, i.ownership, i.note_set_id, i.ordinal, i.kind,\n\
                    ...
                    bm25(memory_units_fts) AS memory_rank\n\
             FROM memory_units_fts\n\
             JOIN memory_units i\n\
               ON i.vault_id = memory_units_fts.vault_id\n\
              AND i.id = memory_units_fts.memory_id\n\
             WHERE memory_units_fts.vault_id = ",
        );
        query.push_bind(context.id().to_string());
        query.push(" AND memory_units_fts MATCH ");
    ...
    pub async fn publish_explicit(
    ...
    ) -> Result<UnitBundle, StateError> {
        self.ensure_vault_context(context).await?;
        validate_current_bundle(context, bundle)?;
        if bundle.memory.ownership != UnitOwnership::Explicit || bundle.note_set.is_some() {
            return Err(StateError::InvalidInput(
        ...
        match (current.as_ref(), expected_revision) {
            ...
            (Some(current), Some(expected))
                if current.memory.ownership == UnitOwnership::Explicit
        ...
        if let Some((key, request_hash)) = idempotency {
            ...
            sqlx::query(
                "INSERT INTO memory_unit_idempotency\n\
            ...
            .await?;
            sqlx::query(
                "DELETE FROM memory_unit_reservations\n\
    ...
    pub async fn restore_explicit_projection(
        ...
        if self
            ...
            .await?
            .is_some_and(|current| current.memory.ownership != UnitOwnership::Explicit)
        {
            return Err(StateError::Conflict);
        }
        self.ensure_explicit_canonical_current(context, &bundle.memory)
    ...
    pub async fn explicit_idempotency(
        ...
        let row = sqlx::query_as::<_, (String, String)>(
            "SELECT request_hash, memory_id FROM memory_unit_idempotency\n\
             WHERE vault_id = ? AND idempotency_key = ?",
        )
        .bind(context.id().to_string())
        .bind(key)
    ...
    pub async fn reserve_explicit(
        ...
        key: &str,
        request_hash: &str,
    ) -> Result<UnitExplicitReservation, StateError> {
        ...
        self.ensure_vault_context(context).await?;
        let proposed = UnitExplicitReservation {
            request_hash: request_hash.to_owned(),
            memory_id: MemoryId::new(),
            created_at: now_millis()?,
        };
        sqlx::query(
            "INSERT INTO memory_unit_reservations\n\
             (vault_id, idempotency_key, request_hash, memory_id, created_at)\n\
             VALUES (?, ?, ?, ?, ?)\n\
    ...
    ) -> Result<Option<UnitSourceSetRecord>, StateError> {
        self.ensure_vault_context(context).await?;
        let row = sqlx::query_as::<_, UnitSourceSetRow>(&format!(
    ...
    pub async fn list_note_set_items(
        ...
        context: &VaultContext,
        set_id: MemorySetId,
    ) -> Result<Vec<UnitBundle>, StateError> {
        self.ensure_vault_context(context).await?;
        let sql = format!(
    ...
    pub async fn prepare_note_set_snapshot(
        ...
        sqlx::query(
            "INSERT INTO memory_unit_snapshots\n\
    ...
    pub async fn prepared_note_set_snapshot(
        ...
        context: &VaultContext,
        source_file_id: FileId,
    ) -> Result<Option<UnitSourceSetSnapshotRecord>, StateError> {
        self.ensure_vault_context(context).await?;
        let row = sqlx::query_as::<_, UnitSourceSetSnapshotRow>(&format!(
    ...
    pub async fn reject_note_set_snapshot(
        ...
        let result = sqlx::query(
            "UPDATE memory_unit_snapshots SET status = 'rejected'\n\
    ...
    pub async fn publish_note_set(
        ...
        for item in items {
            ...
            if item.memory.ownership != UnitOwnership::NoteDerived
        ...
        self.ensure_note_set_files_current(context, note_set, true)
            .await?;
        let snapshot = self
        ...
        .await?;
        if paused && snapshot.provider_id.is_some() {
            return Err(StateError::Conflict);
        }
        let revision: Option<i64> = sqlx::query_scalar(
            "SELECT set_revision FROM memory_unit_sets WHERE vault_id=? AND source_file_id=?",
        )
        .bind(context.id().to_string())
        ...
        let current_files: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM file_entries s JOIN file_entries c ON c.vault_id=s.vault_id WHERE s.vault_id=? AND s.id=? AND s.content_hash=? AND s.current_revision=? AND s.deleted_at IS NULL AND c.id=? AND c.current_revision=? AND c.path=? AND c.deleted_at IS NULL)")
            .bind(context.id().to_string()).bind(note_set.source_file_id.to_string()).bind(&note_set.source_content_hash).bind(note_set.source_revision.as_i64()?)
            .bind(note_set.canonical_file_id.to_string()).bind(note_set.canonical_revision.as_i64()?).bind(note_set.canonical_path.as_str()).fetch_one(&mut *transaction).await?;
        if !current_files {
            return Err(StateError::Conflict);
        }
        delete_set_fts(&mut transaction, context.id(), note_set.id).await?;
        if current.is_some() {
            sqlx::query("DELETE FROM memory_units WHERE vault_id = ? AND note_set_id = ?")
                .bind(context.id().to_string())
    ...
    pub async fn restore_note_set_projection(
        ...
        self.ensure_note_set_files_current(context, note_set, !items.is_empty())
            .await?;
        let current = self
            .get_note_set_by_source(context, note_set.source_file_id)
            .await?;
        ...
        for item in items {
            if self
                ...
                .await?
                .is_some_and(|stored| {
                    stored.memory.ownership != UnitOwnership::NoteDerived
                        || !matches!(
        ...
        if revision
            != current
                .as_ref()
                .map(|set| set.set_revision.as_i64())
                .transpose()?
        {
            return Err(StateError::Conflict);
        }
        let files_current: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM file_entries s JOIN file_entries c ON c.vault_id=s.vault_id WHERE s.vault_id=? AND s.id=? AND (?=0 OR s.content_hash=?) AND s.deleted_at IS NULL AND c.id=? AND c.path=? AND c.current_revision=? AND c.deleted_at IS NULL)")
            .bind(context.id().to_string()).bind(note_set.source_file_id.to_string()).bind(!items.is_empty()).bind(&note_set.source_content_hash)
            .bind(note_set.canonical_file_id.to_string()).bind(note_set.canonical_path.as_str()).bind(note_set.canonical_revision.as_i64()?).fetch_one(&mut *transaction).await?;
        if !files_current {
            return Err(StateError::Conflict);
        }
        if let Some(current) = current.as_ref() {
            ...
            if current.id == note_set.id {
                ...
                sqlx::query("DELETE FROM memory_unit_sets WHERE vault_id = ? AND id = ?")
                    .bind(context.id().to_string())
                    .bind(current.id.to_string())
                    .execute(&mut *transaction)
                    .await?;
        ...
        sqlx::query(
            "UPDATE memory_unit_snapshots SET status = 'rejected', applied_at = ?
    ...
    pub async fn delete_note_item_and_pause(
        ...
        updated_set: &UnitSourceSetRecord,
    ) -> Result<(), StateError> {
        self.ensure_vault_context(context).await?;
        ...
        self.ensure_note_set_files_current(context, updated_set, true)
            .await?;
        let item = self
            .get_unchecked(context, memory_id)
            .await?
    ...
    async fn list_sources(
        ...
        context: &VaultContext,
        memory_id: MemoryId,
    ) -> Result<Vec<UnitSourceRecord>, StateError> {
        let rows = sqlx::query_as::<_, UnitSourceRow>(
        ...
        let exists: i64 = sqlx::query_scalar(
            "SELECT EXISTS(\n\
                ...
                WHERE source.vault_id = ? AND source.id = ?\n\
                  AND (? = 0 OR source.content_hash = ?) AND source.deleted_at IS NULL\n\
...
fn contribution_eligibility_sql() -> &'static str {
    "(\n\
        ...
        (i.ownership = 'note_derived' AND EXISTS (\n\
            SELECT 1 FROM memory_unit_sets s\n\
...
fn current_eligibility_sql() -> String {
    contribution_eligibility_sql().to_owned()
}

fn append_filter<'a>(query: &mut QueryBuilder<'a, Sqlite>, filter: &'a UnitFilter) {
    ...
    if let Some(source_path) = filter.source_path.as_deref() {
        query.push(
            " AND EXISTS (SELECT 1 FROM memory_unit_sources source\n\
    ...
    if let Some(prefix) = filter.path_prefix.as_deref() {
        ...
        query.push(" AND EXISTS (SELECT 1 FROM memory_unit_sources src JOIN file_entries f ON f.vault_id=src.vault_id AND f.id=src.note_file_id WHERE src.vault_id=i.vault_id AND src.memory_id=i.id AND f.deleted_at IS NULL AND (f.path=");
        query.push_bind(prefix);
        query.push(" OR substr(f.path,1,");
        query.push_bind(prefix.chars().count() as i64 + 1);
        query.push(")=");
        query.push_bind(format!("{prefix}/"));
        query.push("))");
    }
    for topic in &filter.topic_ids {
        query.push(" AND EXISTS(SELECT 1 FROM memory_unit_sources src JOIN index_memberships member ON member.vault_id=src.vault_id AND member.file_id=src.note_file_id JOIN index_nodes node ON node.vault_id=member.vault_id AND node.id=member.node_id WHERE src.vault_id=i.vault_id AND src.memory_id=i.id AND node.node_type IN ('topic','manual_topic','tag') AND node.stable_key=");
        query.push_bind(topic);
        query.push(")");
    }
...
async fn upsert_item(
    ...
    let statement = "INSERT INTO memory_units\n\
...
async fn replace_sources(
    ...
    sqlx::query("DELETE FROM memory_unit_sources WHERE vault_id = ? AND memory_id = ?")
...
async fn insert_item_fts_raw(
    ...
    .bind(memory_search_terms(
        std::iter::once(item.metadata["retrieval_hint"].as_str().unwrap_or(""))
...
async fn delete_item_fts_raw(
    ...
    sqlx::query("DELETE FROM memory_units_fts WHERE vault_id = ? AND memory_id = ?")
        .bind(vault_id.to_string())
        .bind(memory_id.to_string())
        .execute(&mut **transaction)
        .await?;
...
async fn update_note_set(
    ...
    let mut query = QueryBuilder::<Sqlite>::new("UPDATE memory_unit_sets SET source_path = ");
...
fn validate_current_bundle(context: &VaultContext, bundle: &UnitBundle) -> Result<(), StateError> {
...
fn validate_note_set(context: &VaultContext, set: &UnitSourceSetRecord) -> Result<(), StateError> {
...
#[derive(Debug, FromRow)]
struct UnitSourceSetSnapshotRow {
    id: String,
    vault_id: String,
    note_set_id: String,
    source_file_id: String,
...
fn row_to_item(row: UnitRow) -> Result<UnitRecord, StateError> {
    Ok(UnitRecord {
        id: MemoryId::parse(&row.id)?,
        vault_id: VaultId::parse(&row.vault_id)?,
        ownership: UnitOwnership::parse(&row.ownership)?,
        note_set_id: row
            .note_set_id
            .as_deref()
...
fn row_to_source(row: UnitSourceRow) -> Result<UnitSourceRecord, StateError> {
    Ok(UnitSourceRecord {
...
fn row_to_set(row: UnitSourceSetRow) -> Result<UnitSourceSetRecord, StateError> {
    Ok(UnitSourceSetRecord {
...
fn row_to_snapshot(
    row: UnitSourceSetSnapshotRow,
) -> Result<UnitSourceSetSnapshotRecord, StateError> {
    Ok(UnitSourceSetSnapshotRecord {
